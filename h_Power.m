function [Power, ratioPower, stats_out, test_out] = h_Power(ft_trialified, tvec_in, tvec_out, frequencies, samplerate, wavelet_cycles, baseline, subsample, save_stats, cfg_test)
    ...
    % Initialize
    Power = zeros(n_frequencies,length(timepoints_of_interest));
    ...
    for fi = 1:n_frequencies
        ...
        % Extract Power
        hold = mean(abs(conv_data).^2,2);
        Power(fi,:) = hold(timepoints_of_interest);
    ...
    if ~isempty(baseline)
        ...
        ratioPower = zeros(n_frequencies, length(timepoints_of_interest));
