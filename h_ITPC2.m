function [ITPC, ratioITPC, stats_out, test_out] = h_ITPC2(ft_trialified, tvec_in, tvec_leftedge2firstevent, frequencies, samplerate, wavelet_cycles, baseline,  df_timepoints, mintimes, subsample, save_stats, cfg_test)
    ...
    % Initialize
    ITPC = zeros(n_frequencies,mintimes);
    ratioITPC = zeros(n_frequencies,mintimes);
    stats_out = [];
    test_out = [];
    ...
    for fi = 1:n_frequencies
        ...
        % Extract ITPC for subsampled time points
        allpoints = exp(1i*angle(conv_data)); hold = zeros(n_trials,mintimes);
        for ti = 1:n_trials
            timepoints_of_interest = dsearchn(tvec_in',df_timepoints{ti}');
            hold(ti,:) = allpoints(timepoints_of_interest,ti);
        end
        ITPC(fi,:) = abs(mean(hold));
    ...
    if ~isempty(baseline)
        ...
        for fi=1:n_frequencies
            bl_avg  = mean(ITPC(fi,bl_points(1):bl_points(2)));
            ratioITPC(fi,:) = ITPC(fi,:) / bl_avg;
        end
    ...
    if ~isempty(cfg_test)
        ...
        if any(cfg_test.method == 'ratio')
            ...
            shuff = zeros(n_frequencies, mintimes, cfg_test.iters); % Shuffling data frame
