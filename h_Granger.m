function [XtoY, YtoX, ratioXtoY, ratioYtoX, percXtoY, percYtoX, zXtoY, zYtoX, rangeXtoY, rangeYtoX, stats_out, test_outXtoY, test_outYtoX] = h_Granger(ft_trialifiedX, ft_trialifiedY, order, tvec_in, tvec_in_samplerate, tvec_out, ...
                                                                                                                                                       timewindow, frequencies, baseline, subtractERP, subsample, save_stats, cfg_test)
    ...
    if (subtractERP)
        ...
        df_ERP_X = zeros(n_trials,n_timepoints_trial); df_ERP_Y = zeros(n_trials,n_timepoints_trial);
        for t = 1:n_trials
    ...
    for toi = 1:length(timepoints_of_interest)
        ...
        % So, we're gonna grab the time window from each trial and line them all up, 
