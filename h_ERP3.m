function [ERPs, ERPs_ft, tvec] = h_ERP3(epoched_lfps, df_task, subIDs, conditions, normalization, baseline)
    ...
    for c = conditions
        ...
        for s = 1:length(subIDs) % ..for each subject's session
            ...
            % Trials of Interest
            toi = logical(getfield(df_task{s},c));
            ...
            for t = 1:sum(toi)
                ...
                if (normalization == "z-score")
                    chosen_trials{t} = normalize(chosen_trials{t});
                elseif (normalization == "range")
                    chosen_trials{t} = normalize(chosen_trials{t},"range");
                end
